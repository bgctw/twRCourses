...
(tmp <- rmd2pdf("RErrorHandling.Rmd"))
setWd(prevWd)

.tmp.f <- function(){} 
	# source out into package beamerR
	library(knitr)
	
	tryCatch({
		prevWd <- setwd("inst/errorHandling")
	
		knit('RErrorHandling.Rmd')
		...
		#system(cmd)
		#copy2clip(cmd <- "sed -i 's/\\\\\\\\end{verbatim}/\\\\\\\\end{verbatim}{}\\\\n\\\\\\\\end{block}/g' RErrorHandlingSlides.tex")
		#system(cmd)	
